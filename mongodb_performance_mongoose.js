...
class Test{
    ...
    async insertBatch(){
        ...
        let start = new Date();
        let ret = await this.collection.insertMany(retList);
...
(async () => {
    ...
    for(let i=0;i<2000;i++){
        let stats = await mongoose.connection.db.stats();
        let _stats = {
        ...
        for(let k in tables){
            let num = tables[k]
            if(i/20!=0 && num > 10000)num = 100;
            let test = new Test(mongoose.connection.db,appId,k,num,i);
            await test.main();
        }
    }
